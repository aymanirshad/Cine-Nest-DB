...
(16, 'Black Panther', 'T''Challa, heir to the hidden but advanced kingdom of Wakanda, must step forward to lead his people into a new future.', 2018, 'photos/slider-bg.jpg'),
(17, 'The Dark Knight', 'When the menace known as the Joker wreaks havoc and chaos on the people of Gotham, Batman must accept one of the greatest psychological tests.', 2008, 'photos/slider-bg2.jpg'),
...
(21, 'Oppenheimer', 'The story of J. Robert Oppenheimer.', 2023, 'photos/mv-item21.jpg');
...
VALUES ('yousaf', 'yousaf1234', 'Admin');

INSERT INTO Admin (admin_id, account_id, email)
VALUES (2, LAST_INSERT_ID(), 'yousufejaz17@gmail.com');

INSERT IGNORE INTO Account (username, password, role) 
VALUES ('admin', 'admin123', 'Admin');

INSERT INTO Flag (content_type, content_id, reason, reporter_id, status)
VALUES 
('Review', 1, 'Contains spoilers and hate speech', 1, 'Pending'),
('Post', 1, 'Inappropriate language', 1, 'Pending');


-- fucntion for usr genre based recommendation
drop function if exists getfriendrecommendation;

delimiter $$

create function getfriendrecommendation(target_user_id int) 
returns int
deterministic
reads sql data
begin
    declare rec_movie_id int;
    
    select w.movie_id into rec_movie_id
    from watchlist w
    join friendship f 
      on (f.sender_id = w.user_id or f.receiver_id = w.user_id)
    where (f.sender_id = target_user_id or f.receiver_id = target_user_id)
      and f.status = 'Accepted'
      and w.watch_status = 'Completed'
      and w.user_id != target_user_id
      and w.movie_id not in (
...
-- trigger for adding movie
delimiter $$
create trigger after_movie_insert
after insert on movie
for each row
begin
    -- Select the first available admin ID to satisfy the NOT NULL constraint
    declare system_admin_id int;
    select min(admin_id) into system_admin_id from admin;

    insert into AuditLog (action, target_table, admin_id)
    values (concat('added movie: ', new.title), 'movie', system_admin_id);
end$$
delimiter ;

-- trigger for updating movie
delimiter $$
...
begin
    ...
    insert into AuditLog (action, target_table, admin_id)
    values (concat('updated movie: ', old.title), 'movie', system_admin_id);
end$$
delimiter ;

-- trigger for deleting movie
delimiter $$
...
begin
    ...
    insert into AuditLog (action, target_table, admin_id)
    values (concat('deleted movie id: ', old.movie_id), 'movie', system_admin_id);
end$$
delimiter ;

-- avg review rating procedure 
...
begin
    -- this single command handles both insert (new review) and update (edit review)
    insert into review (user_id, movie_id, rating, review_text, review_datetime)
    values (p_user_id, p_movie_id, p_rating, p_comment, now())
    on duplicate key update 
        rating = p_rating, 
        review_text = p_comment, 
        review_datetime = now();
end$$
...
drop user if exists 'cine_user'@'localhost';

create user 'cine_admin'@'localhost' identified by 'AdminPass123!';
create user 'cine_user'@'localhost' identified by 'UserPass123!';

grant all privileges on DBProject_itr2.* to 'cine_admin'@'localhost';

grant select on DBProject_itr2.* to 'cine_user'@'localhost';

grant insert, update, delete on DBProject_itr2.watchlist to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.history to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.review to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.post to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.comment to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.postlike to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.message to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.friendship to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.event to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.eventparticipation to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.notification to 'cine_user'@'localhost';
grant insert, update, delete on DBProject_itr2.userfavgenre to 'cine_user'@'localhost';
